...
IPAddress local_IP(192, 168, 42, 93);
...
const char* device_name = "motor_controller";
...
String mode = "";
byte motID[6] =  {0x05,0x01,0x02,0x03,0x04,0x06}; //max expected motor control from a single node is drive control (6)
...
void sub2topics(){
    ...
    client.subscribe("mode/enter");
    client.subscribe("mode/vel");
    client.subscribe("mode/pos");
    client.subscribe("motor/0");  
    client.subscribe("motor/1"); 
    client.subscribe("motor/2"); 
    client.subscribe("motor/3"); 
    client.subscribe("motor/4"); 
    client.subscribe("motor/5"); 
    client.subscribe("write/motor");  
    client.subscribe("set/zero");
...
void callback(char* topic, byte* payload, unsigned int length) {
 ...
 if (String(topic) == ("mode/exit")) {
 ...
 else if (String(topic) == ("mode/enter")) {
   onButton = 1;
 }
 else if ((String(topic) == ("mode/vel")) && (onFlag==false)) {
   input_mode = "velocity";
   //reads mode for test motor at index 0 from MQTT
   mode = input_mode; 
   client.publish("ack","Velocity Mode Entered");
   Serial.println("Velocity Mode");
 }
 else if ((String(topic) == ("mode/pos")) && (onFlag==false)) {
   input_mode = "position";
   //reads mode for test motor at index 0 from MQTT
   mode = input_mode;
   client.publish("ack","Position Mode Entered");
   Serial.println("Position Mode");
 }

 else if (String(topic)==("set/zero")){
   ...
   client.publish("ack","Position Zeroed");
 }

 //update values asynchonously from MQTT app for testing
 //this array is used for both position and velocity depending on mode set.
 else if (String(topic) == ("motor/0")) {
   motor_values_in[0] = msg.toFloat();
 }
 else if (String(topic) == ("motor/1")) {
   motor_values_in[1] = msg.toFloat();
 }
 else if (String(topic) == ("motor/2")) {
   motor_values_in[2] = msg.toFloat();
 }
 else if (String(topic) == ("motor/3")) {
   motor_values_in[3] = msg.toFloat();
 }
 else if (String(topic) == ("motor/4")) {
   motor_values_in[4] = msg.toFloat();
 }
 else if (String(topic) == ("motor/5")) {
   motor_values_in[5] = msg.toFloat();
 }
 //update all motor values synchronously from ROS/Python
 else if (String(topic) == ("write/motor")) {
...
void loop() {
  ...
  //WILL TOGGLE ON/OFF IF LOGIC PIN NOT CONNECTED!!
      ...
      if (onButton){
        ...
        client.publish("ack","Entered Motor Mode");
      }
      ...
      if (offButton){
        ...
        client.publish("ack","Exited Motor Mode");
      }
...
void unpack_reply(){
  ...
  Serial.print("ID: ");
  publishDouble(id, "read/id",0);
  Serial.println(id);
  Serial.print("position: ");
  publishDouble(p_out[index], "read/pos",2);
  Serial.println(p_out[index]);
  Serial.print("velocity: ");
  publishDouble(v_out[index], "read/vel",2);
  Serial.println(v_out[index]);
  Serial.print("torque: ");
  publishDouble(t_out[index], "read/torq",2);
...
void publishJSON(){ //contains index of reading for motor and ID of motor at that index
    ...
    JsonArray data = doc1.createNestedArray("VPTMotor0");
    data.add(v_out[0]);
    data.add(p_out[0]);
    data.add(t_out[0]);
    data = doc1.createNestedArray("VPTMotor1");
    data.add(v_out[1]);
    data.add(p_out[1]);
    data.add(t_out[1]);
    data = doc1.createNestedArray("VPTMotor2");
    data.add(v_out[2]);
    data.add(p_out[2]);
    data.add(t_out[2]);
    data = doc2.createNestedArray("VPTMotor3");
    data.add(v_out[3]);
    data.add(p_out[3]);
    data.add(t_out[3]);
    data = doc2.createNestedArray("VPTMotor4");
    data.add(v_out[4]);
    data.add(p_out[4]);
    data.add(t_out[4]);
    data = doc2.createNestedArray("VPTMotor5");
    data.add(v_out[5]);
    data.add(p_out[5]);
    data.add(t_out[5]);
    //doc["data"]=data;
    ...
    char out1[400];
    char out2[400];
    int b =serializeJson(doc1, out1);
    int b2 =serializeJson(doc2, out2);
    //Serial.print("bytes = ");//use to ensure no overflow
    //Serial.println(b);
    
    //Serial.println(out1);
    client.publish("read/driveMotors1", out1);
    client.publish("read/driveMotors2", out2);
  }
